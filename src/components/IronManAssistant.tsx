...
/* ────────────────────────────────────────────────────────── */
/*  Facts — rotates through these, then loops                 */
/* ────────────────────────────────────────────────────────── */
const FACTS = [
...
/*  Timings                                                    */
/* ────────────────────────────────────────────────────────── */
const HELMET_APPEAR_DELAY = 1500;     /* helmet fades in 1.5s after mount */
const FIRST_BUBBLE_DELAY = 7000;       /* first bubble at 7s */
const BUBBLE_SHOW_DURATION = 9000;     /* each bubble stays for 9s */
const BUBBLE_REPEAT_INTERVAL = 30000;  /* 30s between bubble appearances */
...
export function IronManAssistant() {
  ...
  /* Bubble cycle — first at 7s, then every 30s. Stops on dismiss. */
  useEffect(() => {
    let cancelled = false;
  ...
  const handleHelmetClick = () => {
    /* Tapping helmet manually triggers the current fact (even if dismissed) */
    if (bubbleVisible) return;
    setBubbleVisible(true);
    /* Allow re-engagement after dismiss */
  ...
  return (
    <div
      className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 pointer-events-none"
      style={{ zIndex: 60, perspective: 800 }}
      aria-hidden={!helmetVisible}
    >
      {/* ── Speech bubble (left of helmet) ── */}
      <div
        className="absolute"
        style={{
          right: "calc(100% - 12px)",
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                filter: "drop-shadow(0 10px 18px rgba(220, 38, 38, 0.45)) drop-shadow(0 4px 8px rgba(0,0,0,0.3))",
...
/* ────────────────────────────────────────────────────────── */
/*  SpeechBubble — comic-book style with tail                  */
/* ────────────────────────────────────────────────────────── */
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow:
              "5px 5px 0 #DC2626, 0 14px 30px -10px rgba(220, 38, 38, 0.35)",
          }}
        >
          {/* Dismiss */}
          <button
            ...
            aria-label="Dismiss"
            className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full flex items-center justify-center transition-colors hover:bg-zinc-100 active:scale-90"
          >
        ...
        >
          ...
          />
          {/* Red drop-shadow line under tail (matches box offset shadow) */}
          <path
            d="M22 13 L4 16"
            stroke="#DC2626"
            strokeWidth="0"
...
/* ────────────────────────────────────────────────────────── */
/*  IronManHelmet — premium cartoon SVG                        */
/* ────────────────────────────────────────────────────────── */
function IronManHelmet() {
  ...
  return (
    ...
    >
      ...
      {/* Animated inner-eye pulse highlight */}
      <motion.g
